...
local function hasSyncSoundRemote()
    return table.find(game:GetService("ReplicatedStorage"):GetDescendants(), function(obj)
        return obj:IsA("RemoteEvent") and obj.Name == "SyncSound"
    end) ~= nil
end                                            
    
if game.PlaceId == 120490664183274 or game.PlaceId == 8502861227 or hasSyncSoundRemote() then
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if key == Settings.Keybinds.INCREASE then
		Settings.Values.radius = math.clamp(Settings.Values.radius + Settings.RADIUS_INCREMENT, 1, 50)
	...
	elseif key == Settings.Keybinds.TOGGLE then
		...
		notify("Reach " .. (Settings.Values.enabled and "Enabled" or "Disabled"))
	...
	elseif key == Settings.Keybinds.INCREASE_HRP then
		Settings.Values.hrpSize = math.clamp(Settings.Values.hrpSize + Settings.HRP_INCREMENT, Settings.HRP_MIN, Settings.HRP_MAX)
	...
	elseif key == Settings.Keybinds.TOGGLE_TEAMMATES then
		...
		notify("Ignore Teammates: " .. (Settings.Values.ignoreTeammates and "ON" or "OFF"))
...
RunService.Heartbeat:Connect(function()
	...
	for _, player in ipairs(Players:GetPlayers()) do
		if player ~= localPlayer then
			if Settings.Values.ignoreTeammates and areTeammates(localPlayer, player) then
				if reachedPlayers[player] then
					resetPlayerHitbox(player)
					reachedPlayers[player] = nil
				end
			...
			if phrp then
				...
				if dist <= Settings.Values.radius and (not phumanoid or phumanoid.Health > 0) then
